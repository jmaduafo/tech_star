...
function TimeDate({
  ...
  function timeIcon(hour: string) {
    if (+hour >= 0 && +hour < 12) {
      return <CiCloudSun className={`${timeFontSize === "dashboard" ? 'w-14 h-14' : ''}`} strokeWidth={0.3}/>
    } else if (+hour >= 12 && +hour < 17) {
      return <CiCloud className={`${timeFontSize === "dashboard" ? 'w-14 h-14' : ''}`} strokeWidth={0.3}/>
    } else if (+hour >=17 && +hour <= 23) {
      return <CiCloudMoon className={`${timeFontSize === "dashboard" ? 'w-14 h-14' : ''}`} strokeWidth={0.3}/>
    }
  }
